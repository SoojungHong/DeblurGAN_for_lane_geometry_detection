...
def subsample(data: Iterable, bounds: Tuple[float, float], hash_fn: Callable, n_buckets=100, salt='', verbose=True):
    ...
    #msg = "Subsampling buckets from {lower_bound} to {upper_bound}, total buckets number is {n_buckets}" #SJ_TEMP_FIX

    if salt:
        msg += f'; salt is {salt}'
        #msg += "; salt is {salt}" #SJ_TEMP_FIX
    if verbose:
        logger.info(msg)
...
def hash_from_paths(x: Tuple[str, str], salt: str = '') -> str:
    ...
    #return sha1("{names}_{salt}".encode()).hexdigest() #SJ_TEMP_FIX
...
def _read_img(x: str):
    ...
    if img is None:
        ...
        #logger.warning("Can not read image {x} with OpenCV, switching to scikit-image") #SJ_TEMP_FIX
        img = imread(x)
    return img
...
class PairedDataset(Dataset):
    def __init__(self,
        ...
        #logger.info("Dataset has been created with {len(self.data_a)} samples") #SJ_TEMP_FIX
    ...
    def _preload(x: str, preload_size: int):
        ...
        if preload_size:
            ...
            #assert min(img.shape[:2]) >= preload_size, "weird img shape: {img.shape}"
        return img
    ...
    def from_config(config):
        ...
        #ORG corrupt_fn = aug.get_corrupt_function(config['corrupt'])
        corrupt_fn = None #SJ_TEST
