...
class MetricCounter:
    ...
    def loss_message(self):
        ...
        #return "; ".join(map(lambda x: "{x[0]}={x[1]:.4f}", metrics)) #SJ_TEMP_FIX
    ...
    def write_to_tensorboard(self, epoch_num, validation=False):
        ...
        for tag in ('G_loss', 'D_loss', 'G_loss_adv', 'G_loss_content', 'SSIM', 'PSNR'):
            ...
            #self.writer.add_scalar("{scalar_prefix}_{tag}", np.mean(self.metrics[tag]), global_step=epoch_num) #SJ_TEMP_FIX
