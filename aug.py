...
def get_transforms(size: int, scope: str = 'geometric', crop='random'):
    augs = {'strong': albu.Compose([albu.HorizontalFlip(),
            ...
            'weak': albu.Compose([albu.HorizontalFlip(), #ORG there was nothing
                                  ]),
            'geometric': albu.OneOf([albu.HorizontalFlip(always_apply=True), #ORG True),
                                     albu.ShiftScaleRotate(always_apply=True), #ORG True),
                                     albu.Transpose(always_apply=True), #ORG True),
                                     albu.OpticalDistortion(always_apply=True), #ORG True),
                                     albu.ElasticTransform(always_apply=True), #ORG True),
                                     ])
            }

    aug_fn = augs[scope]
    crop_fn = {'random': albu.RandomCrop(size, size, always_apply=True),  #ORG True),
               'center': albu.CenterCrop(size, size, always_apply=True)}[crop]  #ORG True)}[crop]
